...
#include "eeprom_emulation_platform_private.h"
 ...
 */
int eeprom_emu_flash_read(struct eeprom_emu_dev *dev, off_t offset, uint8_t *blk, size_t len)
{
        const struct eeprom_emu_config *dev_config = &dev->config;
        int rc;

        rc = dev_config->flashapi.read(dev_config->flash_offset + offset, blk, len);
 ...
 */
int eeprom_emu_flash_write(struct eeprom_emu_dev *dev, off_t offset, const uint8_t *blk, size_t len)
{
        const struct eeprom_emu_config *dev_config = &dev->config;
        int rc;

        rc = dev_config->flashapi.write(dev_config->flash_offset + offset, blk, len);
 ...
 */
int eeprom_emu_flash_erase(struct eeprom_emu_dev *dev, off_t offset, size_t len)
{
        const struct eeprom_emu_config *dev_config = &dev->config;
        int rc;

        rc = dev_config->flashapi.erase(dev_config->flash_offset + offset, len);
 ...
 */
static int eeprom_emu_page_invalidate(struct eeprom_emu_dev *dev, off_t offset)
{
        const struct eeprom_emu_config *dev_config = &dev->config;
        const struct eeprom_emu_data *dev_data = &dev->data;
        uint8_t buf[dev_config->flash_cbs];

        LOG_DBG("Invalidating page at [0x%x]", (ptrdiff_t)offset);
 ...
 */
static uint32_t eeprom_emu_get_address(struct eeprom_emu_dev *dev,
                                       const uint8_t *blk)
{
        const struct eeprom_emu_config *dev_config = &dev->config;
        uint32_t address = 0U;
 ...
 */
static void eeprom_emu_set_change(struct eeprom_emu_dev *dev,
 ...
 */
static int eeprom_emu_is_word_used(struct eeprom_emu_dev *dev, const uint8_t *blk)
{
        const struct eeprom_emu_config *dev_config = &dev->config;
        const struct eeprom_emu_data *dev_data = &dev->data;
 ...
 */
static int eeprom_emu_word_read(struct eeprom_emu_dev *dev, off_t address,
                                uint8_t *data)
{
        const struct eeprom_emu_config *dev_config = &dev->config;
        const struct eeprom_emu_data *dev_data = &dev->data;
...
static int eeprom_emu_flash_get(struct eeprom_emu_dev *dev, struct eeprom_emu_ctx *ctx)
{
        const struct eeprom_emu_config *dev_config = &dev->config;
 ...
 * the old page.
 */
static int eeprom_emu_compactor(struct eeprom_emu_dev *dev,
                                struct eeprom_emu_ctx *ctx)
{
        ...
        LOG_DBG("Compactor called for page at [0x%x]",
 ...
 */
static int eeprom_emu_word_write(struct eeprom_emu_dev *dev, off_t address,
                                 const uint8_t *data,
                                 struct eeprom_emu_ctx *ctx)
{
        const struct eeprom_emu_config *dev_config = &dev->config;
...
static int eeprom_emu_flash_set(struct eeprom_emu_dev *dev, struct eeprom_emu_ctx *ctx)
{
        const struct eeprom_emu_config *dev_config = &dev->config;
        off_t address = ctx->address + ctx->len - ctx->rlen;
        uint8_t *data8 = (uint8_t *)(ctx->data);
...
static int eeprom_emu_range_is_valid(struct eeprom_emu_dev *dev, off_t address,
                                     size_t len)
{
        const struct eeprom_emu_config *dev_config = &dev->config;
...
int eeprom_emu_read(struct eeprom_emu_dev *dev, off_t address, void *data, size_t len)
{
        ...
        /* Handle normal case */
        LOG_DBG("EEPROM read at [0x%x] length[%u]", (ptrdiff_t)address, len);
...
int eeprom_emu_write(struct eeprom_emu_dev *dev, off_t address, const void *data, size_t len)
{
        ...
        /* Handle normal case */
        LOG_DBG("EEPROM write at [0x%x] length[%u]", (ptrdiff_t)address, len);
...
int eeprom_emu_init(struct eeprom_emu_dev *dev)
{
        ...
        if (!device_is_ready(dev_config->flash_dev)) {
                ...
                return -EEPROM_EMU_ENODEV;
        }

        dev_data->flash_erase_val = dev_config->flashapi.erase_value;

        if (dev_config->rambuf == NULL &&
            dev_config->flash_size / dev_config->page_size < 2)
        {
                __ASSERT(0, "At least two pages are required");
                return -EEPROM_EMU_EINVAL;
        }
