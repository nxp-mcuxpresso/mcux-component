...
#if (defined(FSL_FEATURE_SOC_DMA_COUNT) && FSL_FEATURE_SOC_DMA_COUNT)
static void KSDK_LPSCI_DmaCallback(UART0_Type *base, lpsci_dma_handle_t *handle, status_t status, void *userData)
{
    uint32_t event = 0U;
...
static void KSDK_LPSCI_NonBlockingCallback(UART0_Type *base, lpsci_handle_t *handle, status_t status, void *userData)
{
    uint32_t event = 0U;
...
static cmsis_lpsci_resource_t UART0_Resource = {UART0, UART0_GetFreq};
...
static lpsci_dma_handle_t UART0_DmaHandle;
...
static lpsci_handle_t UART0_Handle;
