 ...
 *      fields of SIUL2_0
 *    - MCXE31X devices support only random IV
 *
 *    | Device  | New part revision|
 *    |:-------:|:----------------:|
 *    | MCXE31X | new rev only     |
 *
 */
...
typedef struct
{
     ...
     *                  - For HSE_H/M, IVT or DCD or SELF-TEST or App BSB image; the address may be a QSPI-FLASH (external flash) or system RAM address. <br>
     ...
     *                      - Devices DCD/SELF-TEST Image length must be maximum 8192 bytes (DCD/ST Image header(4 bytes) + maximum DCD/ST Image data (8188 byte))
